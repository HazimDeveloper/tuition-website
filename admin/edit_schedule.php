...
$select_subject_id = mysqli_query($connect,"SELECT * FROM subject WHERE subject_id = '$id_subject'");
$res_subject = mysqli_fetch_assoc($select_subject_id);
// var_dump($res_subject);
...
if(isset($_POST['submit'])){
   ...
   $insert_subject =  mysqli_query($connect,"UPDATE `subject` SET `teacher_id`='$id_teacher',`name`='$name',`time`='$time',`price`='$price',`start_time`='$start_time',`end_time`='$end_time',`Day`='$Day',`category`='$category',`link_gmeet`='$link_gmeet' WHERE subject_id = '$id_subject'");

   if($insert_subject){
    // echo "<script>alert('tea added successfully')</script>";
...
<option selected value="<?= $res_subject['category'] ?>"><?= $res_subject['category'] ?></option>
...
<option value="SPM">SPM</option>
                                    ...
                                    <div class="form-group">
                                        <input type="text" class="form-control form-control-user" id="exampleFirstName"
                                            placeholder="Price .. " name="price" required value="<?= $res_subject['price'] ?>">
                                            <p class="text-danger">Do Not Add RM In Front </p>
                                        </div>
                                    <div class="form-group">
                                ...
                                <label for="">Start Time</label>
                                    <input type="time" class="form-control form-control-user" id="exampleInputEmail"
                                        placeholder="Start Time" name="start_time" value="<?= $res_subject['start_time'] ?>"  required>
                                </div>    
                                <div class="form-group">
                                    <label for="">End Time</label>
                                    <input type="time" class="form-control form-control-user" id="exampleInputEmail"
                                        placeholder="End Time" name="end_time" required value="<?= $res_subject['end_time'] ?>">
                                </div>    
                                <div class="form-group">                       
<select class="form-select rounded p-2" name="day" aria-label="Default select example" required >

<option selected value="<?= $res_subject['Day'] ?>" disabled><?= $res_subject['Day'] ?></option>
                  ...
                  <p class="help-block text-danger"></p>
                                    ...
                                    <div class="form-group">
                                        <input type="text" name="link_gmeet" required class="form-control form-control-user"
                                            id="exampleRepeatPassword" placeholder="Link Gmeet .. " value="<?= $res_subject['link_gmeet'] ?>">
                                    </div>
